...
- pre_write: enforce-triad-hot-surface.py --check → FAIL (STATE_ARCHIVE_REQUIRED; lines 1259/1200)
- rollover: enforce-triad-hot-surface.py --rollover --json → `{"boundary":"triad-rollover|state","moved":1,"pack_ref":"docs/engineering/state-archive/state-pack-20260917-d.md","retained_checkpoints":13,"retained_lines":1188}`
- post_write: enforce-triad-hot-surface.py --check → PASS (after verify-work checkpoint re-append)
